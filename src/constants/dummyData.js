export const data = {
  question: [
    {
      id: "1",
      description: "Which is the lowest class of civil Award in Pakistan?",
      description2: "پاکستان میں سب سے چھوٹا سول ایوارڈ  کونسا ہے؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Nishaan / نشان",
          isTrue: false
        },
        {
          id: "2",
          details: "Hilal / حلال",
          isTrue: false
        },
        {
          id: "3",
          details: "Sitara / ستارہ",
          isTrue: false
        },
        {
          id: "4",
          details: "Tamgha / تمغہ",
          isTrue: true
        }
      ]
    },
    {
      id: "2",
      description:
        "Who was cited with the first Nishan-i-Haider for his gallant and actions of valor?",
      description2:
        "کونسے فوجی کو سب سے پہلے انکی بہادری اور دلیری پر نشان حیدرسے نوازا گیا؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Muhammad Sarwar Shaheed / محمد سرور شہید",
          isTrue: true
        },
        {
          id: "2",
          details: "Aziz bhatti Shaheed / عزیز بھٹی شہید",
          isTrue: false
        },
        {
          id: "3",
          details: "Rashid Minhas Shaheed / راشد منہاس شہید",
          isTrue: false
        },
        {
          id: "4",
          details: "kernel Sher Khan Shaheed / کرنل شیر خان شہید",
          isTrue: false
        }
      ]
    },
    {
      id: "3",
      description:
        "In which year Abdus Salam became the first Pakistani and the first Muslim to be awarded the Nobel Prize in the sciences?",
      description2:
        "عبد السلام کس سن میں سب سے پہلے نوبل پرائز جیتنے والے مسلمان اور پاکستانی بنے؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "1978 / ۱۹۷۸",
          isTrue: false
        },
        {
          id: "2",
          details: "1979 / ۱۹۷۹",
          isTrue: true
        },
        {
          id: "3",
          details: "1980 / ۱۹۸۰",
          isTrue: false
        },
        {
          id: "4",
          details: "1981 / ۱۹۸۱",
          isTrue: false
        }
      ]
    },
    {
      id: "4",
      description: "Pakistan's largest coal reserves are found in which area?",
      description2:
        "پاکستان کے کوئلے کے سب سے بڑے ذخائر کس علاقے میں پائے جاتے ہیں؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Duki / دکی‎",
          isTrue: false
        },
        {
          id: "2",
          details: "Thar / تھر",
          isTrue: true
        },
        {
          id: "3",
          details: "Musakhael / موسی خیل",
          isTrue: false
        },
        {
          id: "4",
          details: "Quetta / کوئٹہ",
          isTrue: false
        }
      ]
    },
    {
      id: "5",
      description: "Which is the smallest river of pakistan?",
      description2: "پاکستان کا سب سے چھوٹا دریا کون سا ہے؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Jehlum / دریائے جہلم",
          isTrue: false
        },
        {
          id: "2",
          details: "Sutlej / دریائے ستلج",
          isTrue: false
        },
        {
          id: "3",
          details: "Ravi / دریائے راوی",
          isTrue: true
        },
        {
          id: "4",
          details: "Chenab / دریائے چناب",
          isTrue: false
        }
      ]
    },
    {
      id: "6",
      description: "In which year, Pakistan was champion in 4 different games?",
      description2:
        "پاکستان کس سال بیک وقت  4 مختلف کھیلوں میں عالمی چیمپیئن تھا؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "1992 / ۱۹۹۲",
          isTrue: false
        },
        {
          id: "2",
          details: "1993 / ۱۹۹۳",
          isTrue: false
        },
        {
          id: "3",
          details: "1994 / ۱۹۹۴",
          isTrue: true
        },
        {
          id: "4",
          details: "1995 / ۱۹۹۵",
          isTrue: false
        }
      ]
    },
    {
      id: "7",
      description: "Who had the shortest tenure as President of Pakistan?",
      description2:
        "پاکستان کے صدر کی حیثیت سے سب سے کم دور حکومت کس کے پاس رہی؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Iskander Mirza / اسکندر مرزا",
          isTrue: false
        },
        {
          id: "2",
          details: "Zulfiqar Ali Bhutto / ذُوالفِقار علی بُھٹّو‎‎",
          isTrue: true
        },
        {
          id: "3",
          details: "Yahya Khan / محمد یحییٰ خان",
          isTrue: false
        },
        {
          id: "4",
          details: "Sheikh Rasheed / شیخ رشید",
          isTrue: false
        }
      ]
    },
    {
      id: "8",
      description: "Who wrote the national anthem of Pakistan?",
      description2: "پاکستان کا قومی ترانہ کس نے لکھا؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Mohammad Hafeez / محمد حفیظ",
          isTrue: false
        },
        {
          id: "2",
          details: "Khattal Hafeez / خطال حفیظ",
          isTrue: false
        },
        {
          id: "3",
          details: "Hafeez Jalandhari / حفیظ جالندھری",
          isTrue: true
        },
        {
          id: "4",
          details: "Koteshwer Hafeez / کوٹیشور حفیظ",
          isTrue: false
        }
      ]
    },
    {
      id: "9",
      description:
        "Which of the following countries is not bordered by Pakistan?",
      description2:
        "مندرجہ ذیل ممالک میں سے کون سے ملک کی سرحد پاکستان کی سرحد سے متصل نہیں ہے؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Bangladesh / بنگلہ دیش",
          isTrue: true
        },
        {
          id: "2",
          details: "India / ہندوستان",
          isTrue: false
        },
        {
          id: "3",
          details: "Afghanistan / افغانستان",
          isTrue: false
        },
        {
          id: "4",
          details: "China / چین",
          isTrue: false
        }
      ]
    },
    {
      id: "10",
      description: "Which tree is the national tree of Pakistan?",
      description2: "پاکستان کا قومی درخت کونسا ہے؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Neem / نیم",
          isTrue: false
        },
        {
          id: "2",
          details: "Sheeshum / شیشم",
          isTrue: false
        },
        {
          id: "3",
          details: "Diyodar / دیودار",
          isTrue: true
        },
        {
          id: "4",
          details: "Date Palm / کھجور کا درخت",
          isTrue: false
        }
      ]
    },
    {
      id: "11",
      description:
        "How many slaves did Hazrat Abu Bakr Siddique Razi ALLAHo Anho set free?",
      description2:
        "حضرت ابوبکر صدیق رضی اللہ تعالی عنہ نے کتنے غلاموں کو آزاد کروایا",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "6",
          isTrue: false
        },
        {
          id: "2",
          details: "7",
          isTrue: true
        },
        {
          id: "3",
          details: "10",
          isTrue: false
        },
        {
          id: "4",
          details: "4",
          isTrue: false
        }
      ]
    },
    {
      id: "12",
      description:
        "In which battle did Hazrat Abu Bakr Siddique Razi ALLAHo Anho sacrificed all his households for the sake of Allah?",
      description2:
        "وہ کونسی غزوہ تھی جس میں حضرت ابوبکر صدیق رضی اللہ تعالیٰ عنہٗ نے اپنے گھر کا تمام سامان اللہ کی راہ میں خرچ کیا",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "battle of hunain / غزوہ حنین",
          isTrue: true
        },
        {
          id: "2",
          details: "battle of khandak / غزوہ خندق",
          isTrue: false
        },
        {
          id: "3",
          details: "battle of khyber / غزوہ خیب",
          isTrue: false
        },
        {
          id: "4",
          details: "battle of tabuk /غزوہ تبوک",
          isTrue: false
        }
      ]
    },
    {
      id: "13",
      description:
        "Which surah's ayat was revealed on denial of Hazrat Abu Bakr Siddique Razi ALLAHo anho taking the responsibility of a Sahaba?",
      description2:
        "ایک صحابی  کی کفالت کے انکار پر اللہ رب العالمین نے قرآن میں حضرت ابوبکر  رضی اللہ عنہ کے لیے قرآن کی کونسی سورت سے آیت نازل کی",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Surah ahzab / سورۃ احزاب",
          isTrue: false
        },
        {
          id: "2",
          details: "Surah tauba / سورۃ توبہ",
          isTrue: false
        },
        {
          id: "3",
          details: "Surah ale imran /سورۃ آل عمران",
          isTrue: false
        },
        {
          id: "4",
          details: "Surah noor / سورہ نور",
          isTrue: true
        }
      ]
    },
    {
      id: "14",
      description:
        "Which companion from the following accepted Islam with the efforts of Hazrat Abu Bakr siddiq Razi ALLAHo anho? ",
      description2:
        "ان میں وہ کون سے صحابی ہیں  جو حضرت ابوبکر صدیق رضی اللہ تعالی عنہ کی کوششوں سے ایمان لائے",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details:
            "Hazrat Abu Ubaida bin Jarrah Raziallah anho / حضرت ابو عبیدہ بن جراح رضی اللہ عنہ",
          isTrue: false
        },
        {
          id: "2",
          details:
            "Hazrat Abdur Rahman bin auf Raziallah anho / حضرت عبدالرحمٰن بن عوف رضی اللہ عنہ",
          isTrue: false
        },
        {
          id: "3",
          details:
            "Hazrat Usman bin Affaan Raziallah anho / حضرت عثمان بن عفان رضی اللہ عنہ",
          isTrue: false
        },
        {
          id: "4",
          details: "All three / تینوں",
          isTrue: true
        }
      ]
    },
    {
      id: "15",
      description:
        "What did hazrat Abu Bakr Razi ALLAHo Anho do after knowing that he is being given the food that was earned by black magic? ",
      description2:
        "ایک شخص نے جب حضرت ابوبکر صدیق رضی اللہ تعالی عنہ کو جادو ٹونہ کی کمائی سے کچھ کھلایا  تو معلوم ہونے پر حضرت ابوبکر رضی اللہ عنہٗ کا کیا ردعمل تھا",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Repentance /استغفار کیا",
          isTrue: false
        },
        {
          id: "2",
          details:
            "Vomited by inserting his finger in his mouth /انگلی منہ میں ڈال کر قے کی",
          isTrue: true
        },
        {
          id: "3",
          details: "Broke relations with that person /اس شخص سے قطع تعلقی کی",
          isTrue: false
        },
        {
          id: "4",
          details: "No response from him /  کوئی رد عمل نہیں کیا",
          isTrue: false
        }
      ]
    },
    {
      id: "16",
      description:
        "Who grabbed the neck with rope of Prophet Sallal Laho Alaihi Wasallam, and Hazrat Abu Bakr Siddique Razi ALLAHo Anho fought for his defense?",
      description2:
        "وہ کون تھا  جس نے آپ صلی اللہ علیہ وآلہ وسلم  کی گردن مبارک  پر  رسی کس دی تھی  حضرت ابوبکر صدیق رضی اللہ تعالی عنہ آپ  صلی اللہ علیہ وسلم  کے دفاع میں اس سے لڑے",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Abu jahl / ابوجہل",
          isTrue: false
        },
        {
          id: "2",
          details: "Umya bin khalf / امیہ بن خلف",
          isTrue: false
        },
        {
          id: "3",
          details: "Iqba bin abi mueet / عقبہ بن ابی معیط",
          isTrue: true
        },
        {
          id: "4",
          details: "Abu lahab / ابو لہب",
          isTrue: false
        }
      ]
    },
    {
      id: "17",
      description:
        "From which part, the last whole of Cave Sur was covered by Hazrat Abu Bakr Siddique Razi ALLAHo Anho?",
      description2:
        "غار ثور میں  آخری سوراخ کو حضرت ابوبکر صدیق رضی اللہ تعالی عنہ نے کس چیز سے ڈھانپا",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "His hands thumb /اپنے ہاتھ کے انگوٹھے سے",
          isTrue: false
        },
        {
          id: "2",
          details: "His heel / اپنی  ایڑی سے ",
          isTrue: true
        },
        {
          id: "3",
          details: "His legs thumb /اپنے پاؤں کے انگوٹھے سے",
          isTrue: false
        },
        {
          id: "4",
          details: "Ankle / ٹخنے سے",
          isTrue: false
        }
      ]
    },
    {
      id: "18",
      description:
        "Four deeds in a day (Fasting, Funeral prayer and burial, Feeding the needy) due to which Prophet Sallal Laho Alaihi Wasallam confirm him the blessing of Jannah. What's the fourth one?",
      description2:
        "ایک ہی دن میں چار عمل( روزہ,  نماز جنازہ تدفین,  مسکین کو کھانا کھلانا) کرنے پر رسول صلی اللہ علیہ وسلم نے حضرت ابوبکر صدیق رضی اللہ عنہ کو جنت کی بشارت دی.  چوتھا   عمل کونسا ہے",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Tahajjud /تہجد",
          isTrue: false
        },
        {
          id: "2",
          details: "Visiting sick  /  مریض کی عیادت ",
          isTrue: true
        },
        {
          id: "3",
          details: "Sadqa /صدقہ",
          isTrue: false
        },
        {
          id: "4",
          details: "Being kind to neighbours  / پڑوسی کے ساتھ حسن سلوک",
          isTrue: false
        }
      ]
    },
    {
      id: "19",
      description:
        "On which occasion Hazrat Abu Bakr Siddiq RaziAllah anho affirmed Prophet sallaho alaihi wasaalam's saying, thus got the title SIDDIQUE.?",
      description2:
        "وہ کونسا موقع تھا جب حضرت ابوبکر صدیق رضی اللہ تعالی عنہ نے رسول اللہ صلی وسلم کی تصدیق کی اور اس پر انہیں صدیق کا لقب دیا گیا",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Battle of Badr /جنگ بدر",
          isTrue: true
        },
        {
          id: "2",
          details: "Mairaj /معراج",
          isTrue: false
        },
        {
          id: "3",
          details: "Conquest of Makkah /فتح مکہ",
          isTrue: false
        },
        {
          id: "4",
          details: "Hajjatul wida /حجۃ الوداع",
          isTrue: false
        }
      ]
    },
    {
      id: "20",
      description:
        "If you would had come under my sword during the battle i would have never spared you..To whom did Hazrat Abu Bakr Siddiq RaziAllah Anho said this (later he accepted Islam)?",
      description2:
        "(اگر  تم جنگ میں میری تلوار کی زد میں آتے تو میں تمہیں کبھی نہ چھوڑتا.  حضرت ابوبکر صدیق رضی اللہ تعالیٰ عنہٗ نے یہ  تاریخی جملہ کن سے کہا (بعد میں وہ ایمان لاۓ",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details:
            "Hazra Usama Razi ALLAHo Anho / حضرت اسامہ رضی اللہ تعالیٰ عنہٗ",
          isTrue: false
        },
        {
          id: "2",
          details:
            "Hazrat Zaid Razi ALLAHo Anho / حضرت زید رضی اللہ تعالیٰ عنہٗ",
          isTrue: false
        },
        {
          id: "3",
          details:
            "Hazrat Abu Sufiyan Razi ALLAHo Anho /  حضرت ابو سفیان رضی اللہ تعالیٰ عنہٗ",
          isTrue: true
        },
        {
          id: "4",
          details:
            "Hazrat Abdur Rehman Razi ALLAHo Anho /حضرت عبدالرحمن رضی اللہ تعالیٰ عنہٗ",
          isTrue: false
        }
      ]
    },
    {
      id: "21",
      description:
        "Jack is taller than Peter, and Bill is shorter than Jack. Which of the following statements would be more accurate?",
      description2:
        "جیک پیٹر سے لمبا ہے اور بل جیک سے چھوٹا۔ اس حساب سے مندرجہ ذیل جملوں میں سے صحیح جملہ بتائیں؟",
      timer: 30.0,
      answer: [
        {
          id: "1",
          details: "Bill is taller than Peter / بل جیک سے لمبا ہے ",
          isTrue: false
        },
        {
          id: "2",
          details: "Peter is taller than Bill / پیٹر جیک سے لمبا ہے",
          isTrue: false
        },
        {
          id: "3",
          details: "Bill is as tall as Peter / بل پیٹر جتنا لمبا ہے",
          isTrue: false
        },
        {
          id: "4",
          details:
            "It is impossible to tell whether Bill or Peter is taller / ہ بتانا ناممکن ہے کہ بل اور پیٹر  میں کون زیادہ لمبا ہے",
          isTrue: true
        }
      ]
    },
    {
      id: "22",
      description:
        "Mary had several cookies. After eating one, she gave half the remainder to her sister. After eating another cookie, she gave half of what was left to her brother. Mary now had only five cookies left. How many cookies did she start with?",
      description2:
        "میری کے پاس بہت سارے بسکٹ ہیں۔ ایک کھانے کے بعد اس نے بچے ہوئے آدھے اپنی بہن کو دے دیے۔ ایک اور کھانے کے بعد اس نے بچے ہوئے آدھے اپنے بھائی کو دے دیے۔ اس کے پاس اب پانچ بسکٹ بچے ہیں۔ تو بتائیں اس کے پاس شروع میں کتنے بسکٹ تھے؟",
      timer: 30.0,
      answer: [
        {
          id: "1",
          details: "22",
          isTrue: false
        },
        {
          id: "2",
          details: "23",
          isTrue: true
        },
        {
          id: "3",
          details: "45",
          isTrue: false
        },
        {
          id: "4",
          details: "46",
          isTrue: false
        }
      ]
    },
    {
      id: "23",
      description: "Some month have 31 days, how many have 28?",
      description2:
        "کچھ مہینوں میں 31 دن ہوتے ہیں، کیا بتا سکتے ہیں 28 کتنے مہینوں میں ہوتے ہیں؟",
      timer: 30.0,
      answer: [
        {
          id: "1",
          details: "1",
          isTrue: false
        },
        {
          id: "2",
          details: "4",
          isTrue: false
        },
        {
          id: "3",
          details: "8",
          isTrue: false
        },
        {
          id: "4",
          details: "12",
          isTrue: true
        }
      ]
    },
    {
      id: "24",
      description:
        "Two people can make two bycycle in two hours, how many people are needed to build 12 bycycles in 6 hours?",
      description2:
        "دو لوگ ایک سائیکل دو گھنٹے میں بنا سکتے ہیں، تو 12 سائیکل 6 گھنٹے میں بنانے کے لئے کتنے لوگ درکار ہیں؟",
      timer: 30.0,
      answer: [
        {
          id: "1",
          details: "1",
          isTrue: false
        },
        {
          id: "2",
          details: "2",
          isTrue: false
        },
        {
          id: "3",
          details: "4",
          isTrue: true
        },
        {
          id: "4",
          details: "6",
          isTrue: false
        }
      ]
    },
    {
      id: "25",
      description:
        "you went to visit a doctor and he gave you 7 pills,  he asked to you to take 1 pill every 30 minutes. how many hours will it take to consume all the pills?",
      description2:
        "آپ ڈاکٹر کے پاس گئے اور اس نے آپ کو 7 گولی دیں۔ اس نے آپ کو ہر آدھے گھنٹے بعد ایک گولی کھانے کا مشورہ دیا ہے۔ تو اس حساب سے یہ سات گولیاں کتنی دیر بعد ختم ہو جائیں گی؟",
      timer: 30.0,
      answer: [
        {
          id: "1",
          details: "7",
          isTrue: false
        },
        {
          id: "2",
          details: "3.5",
          isTrue: false
        },
        {
          id: "3",
          details: "3",
          isTrue: true
        },
        {
          id: "4",
          details: "1",
          isTrue: false
        }
      ]
    },
    {
      id: "26",
      description:
        'A man knocks on the door. The lady inside asks who he is? he answers "My mother-in-law is the mother of your mother-in-law". what is the relationship between the two?',
      description2:
        'ایک آدمی نے دروازہ کھٹکھٹایا۔ اندر موجود خاتون نے پوچھا کہ کون ہے؟ آدمی نے جواب دیا "میری ساس آپ کی ساس کی والدہ ہیں"۔ ان دونوں کے درمیان کیا رشتہ ہے؟ ',
      timer: 30.0,
      answer: [
        {
          id: "1",
          details: "father daughter / باپ بیٹی",
          isTrue: false
        },
        {
          id: "2",
          details: "father-in-law daughter-in-law / سسر بہو",
          isTrue: true
        },
        {
          id: "3",
          details: "mother son / ماں بیٹا",
          isTrue: false
        },
        {
          id: "4",
          details: "Mother-in-law son-in-law / ساس داماد",
          isTrue: false
        }
      ]
    },
    {
      id: "27",
      description:
        "You take two apples from a pile of three apples. How many apples do you have?",
      description2:
        "تین سیب کے ڈھیر میں سے آپ نے دو لے لیے۔ اب آپ کے پاس کتنے سیب ہیں؟ ",
      timer: 30.0,
      answer: [
        {
          id: "1",
          details: "3",
          isTrue: false
        },
        {
          id: "2",
          details: "2",
          isTrue: true
        },
        {
          id: "3",
          details: "1",
          isTrue: false
        },
        {
          id: "4",
          details: "0",
          isTrue: false
        }
      ]
    },
    {
      id: "28",
      description:
        "If you have one bucket with 4 liters of water and another bucket with 7.5 grams of water, how many buckets do you have?",
      description2:
        "اگر آپ کے پاس چار لیٹر پانی سے بھری ایک بالٹی ہے، اور دوسری بالٹی 7.5 گرام پانی سے بھری ہے، آپ کے پاس کتنی بالٹی ہیں ؟",
      timer: 30.0,
      answer: [
        {
          id: "1",
          details: "11.5",
          isTrue: false
        },
        {
          id: "2",
          details: "4.75",
          isTrue: false
        },
        {
          id: "3",
          details: "2",
          isTrue: true
        },
        {
          id: "4",
          details: "none",
          isTrue: false
        }
      ]
    },
    {
      id: "29",
      description:
        "My son was playing with a book and tore out pages 7,8,100,101,222,223. How many pages were torn out?",
      description2:
        "میرا بیٹا کتاب سے کھیل رہا تھا اور اس نے صفحہ نمبر ٧، ٨، ١٠٠، ١۔ ١، ٢٢٢ اور ٢٢٣ پھاڑ دیئے؟ تو اس نے کتنے صفحہ پھاڑے؟",
      timer: 30.0,
      answer: [
        {
          id: "1",
          details: "5",
          isTrue: true
        },
        {
          id: "2",
          details: "6",
          isTrue: false
        },
        {
          id: "3",
          details: "7",
          isTrue: false
        },
        {
          id: "4",
          details: "3",
          isTrue: false
        }
      ]
    },
    {
      id: "30",
      description:
        "Refer to the following equation. 2n=9y(43*19)[14/9(50*43)]. What is the fourth number in the equation?",
      description2:
        "2n=9y(43*19)[14÷9(50*43)] اس مساوات میں چوتھا نمبر کونسا ہے؟",
      timer: 30.0,
      answer: [
        {
          id: "1",
          details: "3",
          isTrue: true
        },
        {
          id: "2",
          details: "9",
          isTrue: false
        },
        {
          id: "3",
          details: "19",
          isTrue: false
        },
        {
          id: "4",
          details: "43",
          isTrue: false
        }
      ]
    },
    {
      id: "31",
      description: "Which indian village do we originate from?",
      description2: "ہندوستان میں ہمارے گاؤں کا کیا نام ہے؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Bhatkal /  بھٹکل",
          isTrue: false
        },
        {
          id: "2",
          details: "Murdeshwar / مرڈیشور",
          isTrue: true
        },
        {
          id: "3",
          details: "Manke / منکے",
          isTrue: false
        },
        {
          id: "4",
          details: "Karnatak / کوناٹک",
          isTrue: false
        }
      ]
    },
    {
      id: "32",
      description:
        "Which word is used to describe drawing room in Nawaithy Language?",
      description2: " ہماری زبان میں ڈرائینگ روم کو کیا کہتے ہیں ؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Kholli / کھولی",
          isTrue: false
        },
        {
          id: "2",
          details: "Wasro /وسرو",
          isTrue: true
        },
        {
          id: "3",
          details: "kamra /کمرہ",
          isTrue: false
        },
        {
          id: "4",
          details: "doni /ڈونی",
          isTrue: false
        }
      ]
    },
    {
      id: "33",
      description: "Who was the first president of Nawait Jamiat ul Muslimeen?",
      description2: " ہمارے جمیعت کے پہلے صدر کا کیا نام تھا؟ ",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Hegdey Shareef Shahul Hameed / ھیگڑے شریف شاہ الحمید ",
          isTrue: false
        },
        {
          id: "2",
          details: "Haji Gawahi Muhiuddin / حاجی گواہی محی الدین",
          isTrue: false
        },
        {
          id: "3",
          details: " Saifuddin Tahir / سیف الدین طاہر ",
          isTrue: true
        },
        {
          id: "4",
          details: "Khattal Mohammad Sukri / خطال محمد سکری",
          isTrue: false
        }
      ]
    },
    {
      id: "34",
      description:
        "Out of below, Bidchol shaikh served as what for Nawait jamat ul muslimeen?",
      description2:
        " بڑچول شیخ  محی الدین صاحب جماعت کے ان میں سے کون سے عہدے پر فائز تھے؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "General secretary /  جنرل سیکرٹری",
          isTrue: false
        },
        {
          id: "2",
          details: "Accountant / محاسب",
          isTrue: false
        },
        {
          id: "3",
          details: "Vise president /نائب صدر",
          isTrue: false
        },
        {
          id: "4",
          details: "All of above / اُپر دیئے ہوئے تمام جوابات",
          isTrue: true
        }
      ]
    },
    {
      id: "35",
      description: "Who won the gold medal among below in sir Syed university?",
      description2:
        " وہ کون سی شخصیت ہیں جنہیں سر سید یونیورسٹی میں گولڈ میڈل سے نوازا گیا؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Khattal imtiaz /خطال امتیاز",
          isTrue: false
        },
        {
          id: "2",
          details: "Khattal farrukh /خطال فرخ",
          isTrue: true
        },
        {
          id: "3",
          details: "Khattal Daniyal /خطال دانیال",
          isTrue: false
        },
        {
          id: "4",
          details: "Durgaah nabigh /درگاہ نبیغ",
          isTrue: false
        }
      ]
    },
    {
      id: "36",
      description: "How many people have been burried in Moach Goth Graveyard?",
      description2: "مانج گوٹھ کے قبرستان میں کتنی قبر ہے؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "61",
          isTrue: true
        },
        {
          id: "2",
          details: "51",
          isTrue: false
        },
        {
          id: "3",
          details: "71",
          isTrue: false
        },
        {
          id: "4",
          details: "81",
          isTrue: false
        }
      ]
    },
    {
      id: "37",
      description: "When was the first prayer offered in Masjid-e-Rehman?",
      description2:
        " نوایت کی پہلی مسجد - مسجدِ رحمان نے پہلی نماز کب آدا کی گئی؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "1 ramadan 2014",
          isTrue: true
        },
        {
          id: "2",
          details: "1 ramadan 2015",
          isTrue: false
        },
        {
          id: "3",
          details: "1 ramadan 2016",
          isTrue: false
        },
        {
          id: "4",
          details: "1 ramadan 2017",
          isTrue: false
        }
      ]
    },
    {
      id: "38",
      description: "When was TH formed?",
      description2: " ٹیلنٹ ہنٹرس ( Talent hunters)  کب تشکیل ہوئی؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "2016",
          isTrue: false
        },
        {
          id: "2",
          details: "2017",
          isTrue: false
        },
        {
          id: "3",
          details: "2018",
          isTrue: true
        },
        {
          id: "4",
          details: "2019",
          isTrue: false
        }
      ]
    },
    {
      id: "39",
      description:
        'Who is the author of the poem "Chalye jamat k sath sath chalye"?',
      description2: "چالئے جماعت کے ساتھ چالئے۔ یہ نظم کس نے لکھی ہے؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "Kochebapu Samad /کوچے باپو صمد",
          isTrue: false
        },
        {
          id: "2",
          details: "Dhinda Mehmood / دہندہ محمود",
          isTrue: false
        },
        {
          id: "3",
          details: "Dhinda Abdul Ghani /دہندہ عبدالغنی",
          isTrue: true
        },
        {
          id: "4",
          details: "None of the above  /  کوئی نہیں",
          isTrue: false
        }
      ]
    },
    {
      id: "40",
      description:
        "For how long has the current president of Nawait Community, Khateeb Hafeez has been associated with nawait Jamiat?",
      description2: " نوايت کے موجودہ صدر کس سال سے نوایت جمیعت کے رُکن ہے؟",
      timer: 20.0,
      answer: [
        {
          id: "1",
          details: "1993",
          isTrue: true
        },
        {
          id: "2",
          details: "1994",
          isTrue: false
        },
        {
          id: "3",
          details: "1995",
          isTrue: false
        },
        {
          id: "4",
          details: "1996",
          isTrue: false
        }
      ]
    }
  ]
};
